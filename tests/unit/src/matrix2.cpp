...
const tst::set set("matrix2", [](tst::suite& suite){
	...
	suite.add("submatrix", []{
		...
		auto m = m0.submatrix<0, 1, 2, 2>();

		tst::check_eq(m.size(), size_t(2), SL);
		tst::check_eq(m.front().size(), size_t(2), SL);

		tst::check_eq(m[0][0], 2, SL);
		tst::check_eq(m[0][1], 3, SL);
		tst::check_eq(m[1][0], 5, SL);
		tst::check_eq(m[1][1], 6, SL);
	});
